      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '80px 32px' }}>
        ...
        {ekip.length === 0 ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(240px,1fr))', gap: 20 }}>
            {ekip.map((u: any) => (
              <div key={u.id} style={{ background: '#0e0e1c', border: '1px solid rgba(255,255,255,.06)', borderRadius: 20, padding: 32, textAlign: 'center' }}>
                ...
                <h3 style={{ fontSize: 16, fontWeight: 700, color: '#e0e0f0', marginBottom: 4 }}>{u.ad_soyad}</h3>
                <p style={{ fontSize: 13, color: '#6366f1', fontWeight: 600, marginBottom: 8 }}>{u.unvan}</p>
                {u.uzmanlik && <p style={{ fontSize: 12, color: '#5d5d7a' }}>{u.uzmanlik}</p>}
              </div>
            ))}
          </div>
        )}
