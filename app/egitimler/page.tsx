      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '80px 32px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(300px,1fr))', gap: 20 }}>
          {egitimler.map((e: any, i: number) => (
            <div key={e.id} style={{ background: '#0e0e1c', border: '1px solid rgba(255,255,255,.06)', borderRadius: 20, padding: 32 }}>
              <div style={{ fontSize: 48, fontWeight: 900, color: 'rgba(99,102,241,.15)', lineHeight: 1, marginBottom: 20 }}>{String(i+1).padStart(2,'0')}</div>
              <h2 style={{ fontSize: 18, fontWeight: 700, color: '#e0e0f0', marginBottom: 12 }}>{e.baslik}</h2>
              <p style={{ fontSize: 14, color: '#5d5d7a', lineHeight: 1.7, marginBottom: 20 }}>{e.aciklama}</p>
