...
export default function SiteNav() {
  ...
  // Drawer link tıkla: önce kapat, sonra git
  ...
  return (
    <>
      ...
      <nav style={{position:'sticky',top:0,zIndex:100,background:'#0a0a0f',borderBottom:'1px solid rgba(245,194,0,.15)'}}>
        <div style={{maxWidth:1280,margin:'0 auto',padding:'0 20px',height:64,display:'flex',alignItems:'center',justifyContent:'space-between',gap:12}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:8,flexShrink:0}}>
            ...
            <button type="button" onClick={() => setAcik(true)} className="snav-burger"
      ...
      {/* OVERLAY */}
      <div ref={overlayRef} onClick={kapat}
        style={{display:'none',position:'fixed',inset:0,background:'rgba(0,0,0,.75)',zIndex:9998}} />
        ...
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',
          ...
          <button type="button" onClick={kapat}
            style={{background:'rgba(255,255,255,.06)',border:'1px solid rgba(255,255,255,.12)',
              borderRadius:8,color:'#fff',cursor:'pointer',width:38,height:38,fontSize:20,
              display:'flex',alignItems:'center',justifyContent:'center'}}>✕</button>
        </div>

        {/* Linkler — window.location ile garantili navigation */}
        ...
        <div style={{padding:20,display:'flex',flexDirection:'column',gap:10,
          ...
          <button type="button" onClick={() => git(user ? '/firmalar' : '/giris')}
            style={{padding:14,borderRadius:10,background:'#f5c200',color:'#0a0a0f',
              fontSize:15,fontWeight:800,border:'none',cursor:'pointer',fontFamily:'inherit',textAlign:'center'}}>
          ...
          <button type="button" onClick={() => git('/tehlike-sinifi')}
            style={{padding:12,borderRadius:10,background:'rgba(245,194,0,.08)',
              ...
              fontSize:14,fontWeight:700,cursor:'pointer',fontFamily:'inherit',textAlign:'center'}}>
